...
void database::reindex( const fc::path& data_dir, const fc::path& shared_mem_dir, uint64_t shared_file_size )
{
   try
   {
      ...
      SMOKE_ASSERT( _block_log.head(), block_log_exception, "No blocks in block log. Cannot reindex an empty chain." );
...
{
   return get< reward_fund_object, by_name >( SMOKE_POST_REWARD_FUND_NAME );
}
...
uint32_t database::witness_participation_rate()const
{
   ...
   return uint64_t(SMOKE_100_PERCENT) * dpo.recent_slots_filled.popcount() / 128;
}
   ...
   )
{
   ...
   auto maximum_block_size = get_dynamic_global_properties().maximum_block_size; //SMOKE_MAX_BLOCK_SIZE;
   ...
   {
      FC_ASSERT( fc::raw::pack_size(pending_block) <= SMOKE_MAX_BLOCK_SIZE );
   }
...
void database::pop_block()
{
   try
   {
      ...
      SMOKE_ASSERT( head_block.valid(), pop_empty_chain, "there are no blocks to pop" );
...
void database::notify_pre_apply_operation( operation_notification& note )
{
   ...
   SMOKE_TRY_NOTIFY( pre_apply_operation, note )
}

void database::notify_post_apply_operation( const operation_notification& note )
{
   SMOKE_TRY_NOTIFY( post_apply_operation, note )
}
...
{
   SMOKE_TRY_NOTIFY( applied_block, block )
}

void database::notify_on_pending_transaction( const signed_transaction& tx )
{
   SMOKE_TRY_NOTIFY( on_pending_transaction, tx )
}

void database::notify_on_pre_apply_transaction( const signed_transaction& tx )
{
   SMOKE_TRY_NOTIFY( on_pre_apply_transaction, tx )
}

void database::notify_on_applied_transaction( const signed_transaction& tx )
{
   SMOKE_TRY_NOTIFY( on_applied_transaction, tx )
}
...
fc::time_point_sec database::get_slot_time(uint32_t slot_num)const
{
   ...
   auto interval = SMOKE_BLOCK_INTERVAL;
   const dynamic_global_property_object& dpo = get_dynamic_global_properties();
...
uint32_t database::get_slot_at_time(fc::time_point_sec when)const
{
   ...
   if( when < first_slot_time )
      return 0;
   return (when - first_slot_time).to_seconds() / SMOKE_BLOCK_INTERVAL + 1;
}
...
void database::adjust_proxied_witness_votes( const account_object& a,
                                   ...
                                   int depth )
{
   if( a.proxy != SMOKE_PROXY_TO_SELF_ACCOUNT )
   {
      /// nested proxies are not supported, vote will not propagate
      if( depth >= SMOKE_MAX_PROXY_RECURSION_DEPTH )
         return;
      ...
      {
         for( int i = SMOKE_MAX_PROXY_RECURSION_DEPTH - depth - 1; i >= 0; --i )
         {
   ...
   else
   {
      ...
      for( int i = SMOKE_MAX_PROXY_RECURSION_DEPTH - depth; i >= 0; --i )
         total_delta += delta[i];
...
{
   if( head_block_num() >= SMOKE_OWNER_AUTH_HISTORY_TRACKING_START_BLOCK_NUM )
   {
   ...
   while( current != widx.end() && current->next_vesting_withdrawal <= head_block_time() )
   {
      ...
      */
           ...
           ++itr )
      {
         if( itr->auto_vest )
         {
            share_type to_deposit = ( ( fc::uint128_t ( to_withdraw.value ) * itr->percent ) / SMOKE_100_PERCENT ).to_uint64();
      ...
      modify( from_account, [&]( account_object& a )
      {
         ...
         {
            a.next_vesting_withdrawal += fc::seconds( SMOKE_VESTING_WITHDRAW_INTERVAL_SECONDS );
         }
      });
...
share_type database::cashout_comment_helper( util::comment_reward_context& ctx, const comment_object& comment )
{
  try
  {
     ...
     if( comment.net_rshares > 0 )
     {
        ...
        if( reward_tokens > 0 )
        {
           share_type curation_tokens = ( ( reward_tokens * get_curation_rewards_percent( comment ) ) / SMOKE_100_PERCENT ).to_uint64();
...
share_type database::pay_reward_funds( share_type reward )
{
   ...
   for( auto itr = reward_idx.begin(); itr != reward_idx.end(); ++itr )
   {
      ...
      auto r = ( reward * itr->percent_content_rewards ) / SMOKE_100_PERCENT;
...
void database::account_recovery_processing()
{
   ...
   while( hist != hist_idx.end() && time_point_sec( hist->last_valid_time + SMOKE_OWNER_AUTH_RECOVERY_PERIOD ) < head_block_time() )
   {
      remove( *hist );
      hist = hist_idx.begin();
   }
...
void database::process_decline_voting_rights()
{
   ...
   while( itr != request_idx.end() && itr->effective_date <= head_block_time() )
   {
      ...
      {
         a.can_vote = false;
         a.proxy = SMOKE_PROXY_TO_SELF_ACCOUNT;
      });
...
void database::init_genesis( uint64_t init_supply )
{
   try
   {
      ...
      public_key_type      init_public_key(SMOKE_INIT_PUBLIC_KEY);

      create< account_object >( [&]( account_object& a )
      {
         a.name = SMOKE_MINER_ACCOUNT;
      } );
      ...
      for( int i = 0; i < SMOKE_MAX_WITNESSES; ++i )
      {
         ...
         create< account_authority_object >( [&]( account_authority_object& auth )
         {
            auth.account = SMOKE_INIT_MINER_NAME + ( i ? fc::to_string( i ) : std::string() );
         ...
         {
            w.owner        = SMOKE_INIT_MINER_NAME + ( i ? fc::to_string(i) : std::string() );
      ...
      {
         p.current_witness = SMOKE_INIT_MINER_NAME;
...
//      get_changed_ids( ids );
//      SMOKE_TRY_NOTIFY( changed_objects, ids )*/
//      /*
//      if( _undo_db.enabled() )
//      {
...
//         }
//         SMOKE_TRY_NOTIFY( changed_objects, changed_ids )
//      }
//      */
//   }
//   FC_CAPTURE_AND_RETHROW()
...
{ try {
   ...
   if( block_size < SMOKE_MIN_BLOCK_SIZE )
   {
      elog( "Block size is too small",
         ("next_block_num",next_block_num)("block_size", block_size)("min",SMOKE_MIN_BLOCK_SIZE)
      );
   }
...
{ try {
   ...
   const chain_id_type& chain_id = SMOKE_CHAIN_ID;
   auto trx_id = trx.id();
   ...
   if( !(skip & (skip_transaction_signatures | skip_authority_check) ) )
   {
      ...
      try
      {
         trx.verify_authority( chain_id, get_active, get_owner, get_posting, SMOKE_MAX_SIG_CHECK_DEPTH );
      }
   ...
   if( BOOST_LIKELY(head_block_num() > 0) )
   {
      if( !(skip & skip_tapos_check) )
      {
         ...
         SMOKE_ASSERT( trx.ref_block_prefix == tapos_block_summary.block_id._hash[1], transaction_tapos_exception,
...
{ try {
   ...
   {
      SMOKE_ASSERT( _dgp.head_block_number - _dgp.last_irreversible_block_num  < SMOKE_MAX_UNDO_HISTORY, undo_database_exception,
                 "The database does not have enough undo history to support a blockchain with so many missed blocks. "
                 "Please add a checkpoint if you would like to continue applying blocks beyond this point.",
                 ("last_irreversible_block_num",_dgp.last_irreversible_block_num)("head", _dgp.head_block_number)
                 ("max_undo",SMOKE_MAX_UNDO_HISTORY) );
   }
} FC_CAPTURE_AND_RETHROW() }
...
{ try {
    ...
    *
    */
   if( head_block_num() < SMOKE_START_MINER_VOTING_BLOCK )
   {
      modify( dpo, [&]( dynamic_global_property_object& _dpo )
      {
         if ( head_block_num() > SMOKE_MAX_WITNESSES )
            _dpo.last_irreversible_block_num = head_block_num() - SMOKE_MAX_WITNESSES;
      } );
   }
   else
   {
...
void database::adjust_supply( const asset& delta, bool adjust_vesting )
{
   ...
   const auto& props = get_dynamic_global_properties();
   if( props.head_block_number < SMOKE_BLOCKS_PER_DAY*7 )
...
void database::init_hardforks()
{
   ...
   FC_ASSERT( _hardfork_versions[ hardforks.last_hardfork ] <= SMOKE_BLOCKCHAIN_VERSION, "Blockchain version is older than last applied hardfork" );
   FC_ASSERT( SMOKE_BLOCKCHAIN_HARDFORK_VERSION == _hardfork_versions[ SMOKE_NUM_HARDFORKS ] );
}
...
void database::validate_invariants()const
{
  try
  {
     ...
     for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
     {
        ...
        total_vsf_votes += ( itr->proxy == SMOKE_PROXY_TO_SELF_ACCOUNT ?
   ...
   try
   {
      ...
      for( const auto& account : get_index<account_index>().indices() )
      {
         modify( account, [&]( account_object& a )
         {
            ...
            for( uint32_t i = 0; i < SMOKE_MAX_PROXY_RECURSION_DEPTH; ++i )
...
void database::retally_witness_votes()
{
   ...
   for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
   {
      if( itr->proxy != SMOKE_PROXY_TO_SELF_ACCOUNT ) continue;
...
void database::retally_witness_vote_counts( bool force )
{
   ...
   for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
   {
      ...
      if( force || (a.proxy != SMOKE_PROXY_TO_SELF_ACCOUNT  ) )
      {
        const auto& vidx = get_index< witness_vote_index >().indices().get< by_account_witness >();
        auto wit_itr = vidx.lower_bound( boost::make_tuple( a.id, witness_id_type() ) );
        while( wit_itr != vidx.end() && wit_itr->account == a.id )
